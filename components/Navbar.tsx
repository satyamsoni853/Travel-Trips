...
import { Bars3Icon, XMarkIcon, SunIcon, MoonIcon, ChevronDownIcon } from "@heroicons/react/24/solid";
...
const Navbar = () => {
  ...
  const hoverTextColorClass = isDarkMode ? "hover:text-white" : "hover:text-purple-600";
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          >
              ...
              {NAV_LINKS_UPDATED.map((link) => (
                ...
                >
                  <Link href={link.key === "home" ? "/" : `/${link.key}`}>
                    <span className={`flex items-center gap-1 cursor-pointer rounded-full px-3 py-2 text-[15px] font-medium transition-all duration-300 ${baseTextColor} ${hoverBgClass} ${hoverTextColorClass}`}>
                      {link.label}
                      {link.submenu && <ChevronDownIcon className="h-4 w-4 transition-transform duration-300 group-hover:rotate-180" />}
                    </span>
                  </Link>
                  ...
                  {link.submenu && (
                    <ul 
                      className={`absolute left-0 top-full mt-2 w-48 rounded-2xl shadow-lg border transition-all duration-300 ${
                        isDarkMode ? 'bg-gray-800/90 border-gray-700 backdrop-blur-md' : 'bg-white/90 border-gray-200 backdrop-blur-md' // Added glass effect to dropdown
                      } ${openDropdown === link.key ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none translate-y-2'}`}
                    >
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <button
                onClick={toggleTheme}
                className={`p-2 rounded-full transition-colors duration-300 ${isDarkMode ? 'text-yellow-400 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'}`}
              ...
              <button className={`rounded-full border-2 px-6 py-2.5 text-sm font-semibold transition-all duration-300 
                ${isDarkMode 
      ...
      >
        <ul className="flex flex-col p-4">
            ...
            <li key={link.key} className="mb-2">
              <Link href={link.key === "home" ? "/" : `/${link.key}`}>
                ...
                >
                  {link.label}
                  {link.submenu && 
                      <ChevronDownIcon className={`h-4 w-4 transition-transform duration-300 ${openDropdown === link.key ? 'rotate-180' : 'rotate-0'}`} />
                  }
                </div>
              </Link>
              ...
              {link.submenu && (
                <div 
                    ...
                    className={`overflow-hidden transition-all duration-300 ease-in-out ${
                        openDropdown === link.key ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'
                    }`}
                >
          ...
          {/* Mobile Buttons */}
          <div className={`mt-4 flex flex-col gap-3 pt-4 ${isDarkMode ? "border-t border-gray-700" : "border-t border-gray-100"}`}>
            <button className={`rounded-full border-2 py-3.5 text-sm font-semibold transition-all duration-300 
                ${isDarkMode 
                  ? 'border-gray-600 text-gray-300 hover:border-purple-500 hover:bg-gray-700 hover:text-white' 
                  : 'border-gray-300 text-gray-700 hover:border-purple-600 hover:bg-purple-50 hover:text-purple-600'}`}>
              Login
