...
const CustomButton: React.FC<CustomButtonProps> = ({ title, variant, href = "#" }) => {
    ...
    switch (variant) {
        case "btn_purple":
            activeClasses = "bg-gradient-to-r from-purple-600 to-purple-500 text-white shadow-purple-500/50 hover:shadow-purple-500/70";
            break;
        case "btn_outline":
            activeClasses = "border-2 border-purple-500 text-purple-600 hover:bg-purple-50 hover:text-purple-700";
            break;
        default:
...
const Hero = () => {
    ...
    const badgeBgClass = isDarkMode ? "bg-gray-800 shadow-gray-700/50" : "bg-white shadow-lg";
    ...
    const getEntranceClass = (delay = 0) => 
        `${hasAnimated ? 'slide-in-up' : 'opacity-0 translate-y-4'} transition-all duration-700 ease-out`;
    

    return (
        <section className={`relative max-container pt-32 pb-16 md:pb-24 overflow-hidden ${mainBgClass}`}>
            ...
            <div className="padding-container flex flex-col xl:flex-row items-center justify-between gap-16 xl:gap-0 relative z-10">
                ...
                <div className="relative z-2 flex flex-1 flex-col gap-8 xl:w-2/5 max-w-xl xl:max-w-none text-center xl:text-left">
                    ...
                    <div className={`${getEntranceClass(0)} self-center xl:self-start`} style={{ animationDelay: '0s' }}>
                        <div className={`flex gap-2 w-fit rounded-full py-2 px-6 transition-all duration-300 hover:scale-[1.05] cursor-pointer ${badgeBgClass} ${isDarkMode ? 'text-purple-300' : 'text-purple-600'}`}>
                            <p className="font-semibold text-sm">Explore the world!</p>
                            {/* Simple inline SVG for theme consistency */}
                            <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12 18.269c-2.316 0-4.195-1.879-4.195-4.195S9.684 9.879 12 9.879s4.195 1.879 4.195 4.195-1.879 4.195-4.195 4.195zm0-6.195c-1.102 0-2 .898-2 2s.898 2 2 2 2-.898 2-2-.898-2-2-2zM12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"/></svg>
                        </div>
                    </div>

                    {/* Heading */}
                    <h1 
                        className={`text-5xl md:text-7xl font-extrabold leading-tight ${headingColorClass} ${getEntranceClass(0.15)}`}
                    ...
                    {/* Paragraph */}
                    <p 
                        className={`text-lg md:text-xl ${paragraphColorClass} ${getEntranceClass(0.3)}`}
                    ...
                    {/* Buttons */}
                    <div 
                        className={`flex justify-center xl:justify-start gap-4 mt-4 ${getEntranceClass(0.45)}`}
                        ...
                        {images.map((img, index) => (
                            <Image
                                ...
                                style={{
                                    ...
                                    transform: currentImage === index ? 'translateY(-10px) rotate(-1deg)' : 'translateY(0) rotate(0deg)',
